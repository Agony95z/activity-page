<template>
  <el-tabs v-model="activeName" class="tabs" @tab-click="handleClick">
    <el-tab-pane v-for="item in tabsArr" :key="item.id" :label="item.label" :name="item.name">
      <div class="mb-5">
        <p>{{ item.text }}</p>
        <img class="my-2.5" :src="item.img" alt="">
        <div @click="handleMore" class="flex justify-center text-[#0055D5]">
          了解更多
...
const tabsArr = [
  ...
  {id:4, label: 'xScale', text: 'xScale构建了自主的前端界面+后端计算基础框架，前端界面包括前处理、计算调用、后处理、可视化功能；后端计算包括多个不同尺度的计算力学模块，针对高铁、海洋、航空、盾构等工程结构开展了广泛应用，面向具体需求，可实现高度定制化的应用开发。', img: con_4, name: 'fourth', more: '了解更多'},
];
